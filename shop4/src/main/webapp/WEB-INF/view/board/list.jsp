...
<table>
  <tr><td colspan="5">
	...
	<c:if test="${listcount > 0}">   <%-- 등록된 게시물이 있음 --%>
	  ...
	  <%-- 제목 부분 출력 -> 클릭시 detail.shop으로 이동  --%>
	  	<a href="detail.shop?num=${board.num}">${board.subject}</a></td>
	  		<td>${board.name}</td>
	  <%-- 오늘 게시글이면 시간만, 다른 날짜의 게시글이면 날짜+시간으로 출력 --%>
	  <fmt:formatDate value="${board.regdate}" pattern="yyyyMMdd" var="regdate" />
	  <fmt:formatDate value="${board.regdate}" pattern="HH:mm:ss" var="todayform" />
	  <fmt:formatDate value="${board.regdate}" pattern="yyyy-MM-dd HH:mm" var="nottoday" />
