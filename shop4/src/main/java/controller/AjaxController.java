...
@RestController   // @Controller + @ResponseBody 기능
				  // @ResponseBody : View 없이 직접 데이터를 클라이언트에 전송 -> json
@RequestMapping("ajax")
public class AjaxController {
	@Autowired
	ShopService service;
	
	//graph1.shop 요청이 올때 실행
	...
	public String graph1() {
		...
		json.append("]");
		return json.toString();    //클라이언트에 전달될 데이터(문자열 자체가 전달됨)
	}
	...
	@RequestMapping(value="exchange", produces="text/html; charset=UTF8")
