...
public class UserController {
	...
	public ModelAndView userentry(@Valid User user, BindingResult br) {
		...
		try {
			//1. 비밀번호 : 해시 알고리즘으로 값을 생성 -> db에 저장
			//2. 이메일 : 암호화하여 db에 저장
			//          키는 userid의 해시코드값의 앞 16자리로 설정
			String hashpass = CipherUtil.makehash(user.getPassword());
	...
	public ModelAndView login(@Valid User user, BindingResult br, HttpSession session) {
		...
		try {
			...
			//CipherUtil.makehash(user.getPassword()) : 입력된 비밀번호를 해시값으로 변경
